-- Create trigger to automatically create authority_profiles entries when role invitations are accepted
...
-- Also update the accept_role_invitation function to ensure it works properly
-- (This assumes the function exists and needs to be updated)

-- Create or update the accept_role_invitation function
CREATE OR REPLACE FUNCTION public.accept_role_invitation(invitation_id uuid)
RETURNS void AS $$
DECLARE
