-- Fix the authority profiles function to include profile images
-- First drop the existing function, then create the new one

-- 1. Drop the existing function
DROP FUNCTION IF EXISTS public.get_authority_profiles_for_admin(uuid);

-- 2. Create the new function with profile_image_url
...
-- 4. Test the function (replace with actual authority ID)
-- SELECT profile_email, profile_image_url FROM get_authority_profiles_for_admin('your-authority-id-here') LIMIT 3;