...
-- Replace in_transit with checked_in to match app expectations
-- The app expects: unused -> checked_in -> checked_out
--
-- This script will:
-- 1. Update all passes with in_transit status to checked_in
-- 2. Drop ALL versions of process_pass_movement function (fixes "not unique" error)
-- 3. Create new corrected version with proper status flow
...
-- Step 2: Drop ALL versions of the process_pass_movement function
...
BEGIN
    ...
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS %s CASCADE', func_record.function_signature);
        RAISE NOTICE 'Dropped function: %', func_record.function_signature;
    END LOOP;
END $$;

-- Create the corrected process_pass_movement function
...
-- Create overload with notes parameter
CREATE OR REPLACE FUNCTION process_pass_movement(
    p_pass_id UUID,
    p_border_id UUID,
    p_latitude DECIMAL DEFAULT NULL,
...
BEGIN
    ...
    RAISE NOTICE '   - Updated % passes to checked_in status', updated_count;
    RAISE NOTICE '   - Status flow: unused -> checked_in -> checked_out';
    RAISE NOTICE '   - Function updated to use correct statuses';
END $$;
