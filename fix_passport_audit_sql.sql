-- Add passport document URL column to profiles table (if not exists)
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'profiles' AND column_name = 'passport_document_url') THEN
        ALTER TABLE profiles ADD COLUMN passport_document_url TEXT;
    END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_profile_settings_audit_profile_id ON profile_settings_audit(profile_id);
CREATE INDEX IF NOT EXISTS idx_profile_settings_audit_changed_at ON profile_settings_audit(changed_at);
CREATE INDEX IF NOT EXISTS idx_profile_settings_audit_field_name ON profile_settings_audit(field_name);

-- Enable RLS on audit table
ALTER TABLE profile_settings_audit ENABLE ROW LEVEL SECURITY;

-- Drop existing policies if they exist
DROP POLICY IF EXISTS "Users can view their own audit records" ON profile_settings_audit;
DROP POLICY IF EXISTS "System can insert audit records" ON profile_settings_audit;
DROP POLICY IF EXISTS "Authority users can view audit records in their authority" ON profile_settings_audit;
