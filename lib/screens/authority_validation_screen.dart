...
class _AuthorityValidationScreenState extends State<AuthorityValidationScreen> {
        ...
        children: [
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              onPressed: _backupCodeController.text.length == 9
      ...
      child: Column(
        ...
        children: [
          ...
          _buildDetailCard('Entry Information', [
            _buildDetailRow(
                Icons.confirmation_number, 'Entries', pass.entriesDisplay),
            _buildDetailRow(Icons.attach_money, 'Amount',
                '${pass.currency} ${pass.amount.toStringAsFixed(2)}'),
            _buildDetailRow(
                Icons.calendar_today, 'Expires', _formatDate(pass.expiresAt)),
          ]),
  ...
  Future<void> _validateQRCode(String qrData) async {
    ...
    try {
      ...
      if (pass != null) {
        ...
        setState(() {
          _errorMessage = 'Invalid QR code or pass not found';
          _isProcessing = false;
        });
      }
    } catch (e) {
      setState(() {
        _errorMessage = 'Error validating pass: $e';
        _isProcessing = false;
      });
    }
  }
  ...
  Future<void> _validateBackupCode(String backupCode) async {
    ...
    try {
      ...
      if (pass != null) {
        ...
        setState(() {
          _errorMessage = 'Invalid backup code or pass not found';
          _isProcessing = false;
        });
      }
    } catch (e) {
      setState(() {
        _errorMessage = 'Error validating backup code: $e';
        _isProcessing = false;
      });
    }
  }
  ...
  Future<PurchasedPass?> _parseAndValidatePass(String qrData) async {
    try {
      return await PassService.validatePassByQRCode(qrData);
    } catch (e) {
  ...
  Future<PurchasedPass?> _validatePassByBackupCode(String backupCode) async {
    try {
      return await PassService.validatePassByBackupCode(backupCode);
    } catch (e) {
    ...
    if (text.length <= 4) {
      ...
      return newValue.copyWith(
        ...
        selection: TextSelection.collapsed(
          offset: text.length + (text.length > 4 ? 1 : 0),
        ),
      );
    }
