...
class BorderMovementService {
  ...
  static Future<List<PassMovement>> getBorderMovements(
    ...
    try {
      ...
      // First get the authority_id for this border
      final borderInfo = await supabase
          .from('borders')
          .select('authority_id')
          .eq('id', borderId)
          .single();
      ...
      // For each movement, get the authority profile display name
      ...
      for (final item in response as List) {
        ...
        // Get authority profile display name for this official
        String? officialName = officialProfile?['full_name'];
        if (officialProfileId != null) {
          try {
            final authorityProfileResponse = await supabase
                .from('authority_profiles')
                .select('display_name')
                .eq('authority_id', authorityId)
                .eq('profile_id', officialProfileId)
                .eq('is_active', true)
                .maybeSingle();

            if (authorityProfileResponse != null &&
                authorityProfileResponse['display_name'] != null &&
                authorityProfileResponse['display_name']
                    .toString()
                    .isNotEmpty) {
              officialName = authorityProfileResponse['display_name'];
            }
          } catch (e) {
            // If authority profile lookup fails, use full_name
            print(
                'Could not fetch authority profile for $officialProfileId: $e');
          }
        }
