-- Restore the original verify_pass function (undo the over-engineering)
-- This removes the complex validation logic and extra fields

-- Drop the over-engineered functions
DROP FUNCTION IF EXISTS verify_pass(TEXT, BOOLEAN);
DROP FUNCTION IF EXISTS get_vehicle_location_status(UUID);
DROP FUNCTION IF EXISTS validate_pass_with_location_logic(UUID, TEXT);
DROP FUNCTION IF EXISTS verify_pass_enhanced(TEXT, BOOLEAN, TEXT);

-- Restore the original simple verify_pass function
...
BEGIN
    ...
    -- Step 2: Return the pass details (original simple version)
    RETURN QUERY
    SELECT 
        pp.id,
        pp.profile_id,
...
-- Add comment
COMMENT ON FUNCTION verify_pass IS 'Simple pass verification by QR code (JSON with id) or backup code (short_code/pass_hash). Returns basic pass details without complex validation.';

-- Success message
SELECT '✅ Original simple verify_pass function restored' as status;
SELECT 'Removed over-engineered validation logic and extra fields' as cleanup;